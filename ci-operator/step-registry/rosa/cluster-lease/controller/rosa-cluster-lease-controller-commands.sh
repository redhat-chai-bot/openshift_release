    ...
    if [[ "${RBAC_CHECK_FAILED}" == "true" ]]; then
        if [[ "${STATUS}" != "error" ]] && ! dry_run_guard "Would mark ${CM_NAME} as error (RBAC)"; then
            if ! lease_oc patch configmap "${CM_NAME}" -n "${LEASE_NAMESPACE}" --type merge -p '{
                ...
                log "WARNING: Failed to mark ${CM_NAME} as error after RBAC failure — cluster may still be leaseable, retrying..."
                sleep 3
                if ! lease_oc patch configmap "${CM_NAME}" -n "${LEASE_NAMESPACE}" --type merge -p '{
                    "metadata": {
                        "labels": { "rosa-cluster-lease/status": "error" },
                    ...
                    log "ERROR: Retry failed — could not mark ${CM_NAME} as error after RBAC failure. Cluster may still be leaseable!"
                fi
            fi
        fi
        UNHEALTHY=$((UNHEALTHY + 1))
